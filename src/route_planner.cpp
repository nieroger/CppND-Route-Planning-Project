...
RouteModel::Node *RoutePlanner::NextNode() {
    std::sort(this->open_list.begin(), this->open_list.end(), [](const RouteModel::Node *first, const RouteModel::Node *latter) {
        float f1 = first->g_value + first->h_value;
        float f2 = latter->g_value + latter->h_value;
        return f1 > f2;
    });

    auto closestNode = this->open_list.back();
